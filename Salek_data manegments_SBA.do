...
keep SBA age edu_cat residence  religion_cat wealth_cat child_cat hh_size_group husb_edu health_ins working_status parity delivery_mode ANC_visit birth_size pnc_doc mat_age1st preg_pressure age_first_sex sex_active fert_pref genital_ulcer  media internet_use deci_make province ethnicity any_ipv physical emotional sexual




keep SBA age edu_cat residence ethnicity religion_cat wealth_cat child_cat hh_size_group husb_edu woman_edu health_ins working_status parity delivery_mode ANC_visit birth_size pnc_doc mat_age1st preg_pressure age_first_sex sex_active fert_pref genital_ulcer  media internet_use deci_make IPV WGT province v005 v021 v022
...
save "D:\Research\BDHS Research\Nepal\SBA\Mozambique\Data\BDHS_cleaned_descriptive.dta", replace


save "D:\Research\BDHS Research\Nepal\SBA\Burkina Faso\DataDHS_cleaned_descriptive.dta", replace
...
putdocx text ("Descriptive Statistics of Study Variables among Borkina Faro Reproductive-aged Women (n=5,111)")
...
****************************************************************************************/
/* Maternal variables */
local maternal_vars age mat_age1st age_first_sex parity ANC_visit delivery_mode ///
    birth_size pnc_doc preg_pressure sex_active fert_pref physical sexual emotional any_ipv

/* Education & work */
local edu_work_vars edu_cat husb_edu working_status health_ins

/* Household & socioeconomic variables */
local household_vars province residence  religion_cat wealth_cat ///
    child_cat hh_size_group media internet_use deci_make ethnicity

/* Combine all variables */
local all_vars `maternal_vars' `edu_work_vars' `household_vars'

/****************************************************************************************
     LOOP OVER VARIABLES AND WRITE TO WORD
****************************************************************************************/
local total_wgt = 5833  // Total population (or sum of WGT if different)
...
foreach var of local all_vars {
    ...
    * Get categories
    ...
    foreach cat of local cats {
        ...
        * Weighted N for this category
        qui sum WGT if `var' == `cat'
        local wN = r(sum)
        
        * Percent
        local pct = round((`wN'/`total_wgt')*100, 0.1)
        
        * Get value label
        capture local cat_label : label (`var') `cat'
        if "`cat_label'" == "" local cat_label = "`cat'"
...
putdocx save "D:\Research\BDHS Research\Nepal\SBA\Burkina Faso\Table\Descriptive_StudyVariables.docx", replace
...
*--------------------------------------------
* 1️⃣ Make SBA numeric indicator (1 = Skilled, 0 = Unskilled)
*--------------------------------------------
gen SBA_num = .
replace SBA_num = 1 if SBA == 1
replace SBA_num = 0 if SBA == 0

*--------------------------------------------
* 2️⃣ Collapse to get province-wise total births
*--------------------------------------------
collapse (sum) Skilled=SBA_num (count) Total_births=SBA_num, by(province)

*--------------------------------------------
* 3️⃣ Compute global percentages
*--------------------------------------------
sum Total_births
local grand_total = r(sum)

gen province_pct = round((Total_births / `grand_total')*100,1)
gen Skilled_global_pct = round((Skilled / `grand_total')*100,1)
...
* 4️⃣ Export to Excel
*--------------------------------------------
export excel province Total_births province_pct Skilled Skilled_global_pct ///
    using "D:\Research\BDHS Research\Nepal\SBA\Burkina Faso\Data\division_global_SBA_percentages.xlsx", ///
